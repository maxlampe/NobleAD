...
Base.copy(bl::Beamline) = Beamline(bl.apps_pos, bl.apps_w, bl.det_pos, bl.det_span, bl.apps_maxpos, bl.eps)
...
begin
	...
	theta_probs = [theta_dist(thet) for thet in thetas]
...
function kl_point_loss(
	...
	obs = beam(bl)
	tar = target_dist(bl.det_span)
	# (obs - tar).*(obs - tar)
	min.(kl_divergence.(tar, obs), 1000.)
end	

# ╔═╡ 3dc60b4c-57c6-411c-8946-8bf688bda8f0
function total_loss(bl::Beamline)
	sum_p = sum(kl_point_loss(bl)) / length(bl.det_span)

	pos_s = bl.apps_pos
	mean_w = mean([bl.apps_w...])
	w_s = bl.apps_w./ mean_w

	reg = 0.
	if false
		reg = reg + sum((w_s).* w_s) / bl.n_app
		reg = 0.01 * reg / bl.n_app
	end
...
grad_beam(x::Beamline) = ForwardDiff.gradient(total_loss, x)
...
function train(bl::Beamline)
	println("Start training")

	# Make a copy of original struct Beamline
	bl_old = copy(bl)
...
# ╔═╡ 01a86522-80af-42b4-b9c6-96df1a778b25
begin
